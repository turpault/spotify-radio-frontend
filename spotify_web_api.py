...
from gls_client import GlsConfig, _log_http_error_response
...
def get_me_playlists(
    ...
    """Current user playlists from ``api.spotify.com`` (user OAuth only)."""
    if is_configured():
        ...
        try:
            ...
            _log.warning(
                "me/playlists: Web API call failed (%s); returning no playlists", e
            )
            return []
